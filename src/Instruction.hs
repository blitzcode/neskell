...
decodeOpCode w = case w of
    ...
    ; 0x7A -> OpCode (NOI w) Implied   ; 0x5A -> OpCode (NOI w) Implied   ; 0x1A -> OpCode (NOI w) Implied
    ; 0x3A -> OpCode (NOI w) Implied   ; 0xDA -> OpCode (NOI w) Implied   ; 0xFA -> OpCode (NOI w) Implied
    ; 0x80 -> OpCode (NOI w) Immediate ; 0x82 -> OpCode (NOI w) Immediate ; 0x89 -> OpCode (NOI w) Immediate
    ; 0xC2 -> OpCode (NOI w) Immediate ; 0xE2 -> OpCode (NOI w) Immediate ; 0x04 -> OpCode (NOI w) ZeroPage
    ; 0x64 -> OpCode (NOI w) ZeroPage  ; 0x44 -> OpCode (NOI w) ZeroPage  ; 0x0C -> OpCode (NOI w) Absolute
    ; 0x14 -> OpCode (NOI w) ZeroPageX ; 0x34 -> OpCode (NOI w) ZeroPageX ; 0x54 -> OpCode (NOI w) ZeroPageX
    ...
    ; 0xB3 -> OpCode LAX IndIdx        ; 0x87 -> OpCode SAX ZeroPage      ; 0x97 -> OpCode SAX ZeroPageY     
    ; 0x8F -> OpCode SAX Absolute      ; 0x83 -> OpCode SAX IdxInd        ; _    -> OpCode (DCB w) Implied
...
instance Show Instruction where
    -- Many illegal opcodes are identical in behavior and addressing mode, we
    -- store their binary representation to distinguish them
    show (Instruction (OpCode (DCB b) Implied) []) = printf "DCB #$%02X" b
    show (Instruction (OpCode (KIL b) Implied) []) = printf "KIL #$%02X" b
    show (Instruction (OpCode (NOI b)      am) op) = printf "NOI #$%02X" b ++ showAMAndOP am op
    -- Default, just print Mnemonic and operand
    show (Instruction (OpCode mn           am) op) = show mn               ++ showAMAndOP am op
